...
bool HilogProc_Impl(const HiLogContent *hilogContent, uint32 len)
{
    char tempOutStr[LOG_FMT_MAX_LEN] = {0};
    int32 tmpLen = LogContentFmt(tempOutStr, sizeof(tempOutStr), hilogContent);
    if (tmpLen > 0) {
        hal_trace_output(tempOutStr, tmpLen);
    }
    return TRUE;
}
