...
int do_set_power(int argc, char *argv[])
{
    ...
    if(power_pol == '-') {
        argv[2] = argv[2] + 1;
        tx_pwr_lvl = console_cmd_strtoul(argv[2], NULL, 10);
        if(tx_pwr_lvl > 0x7F) {
            dbg("invalid tx power\n");
            return ERR_WRONG_ARGS;
        }
        tx_pwr_lvl = 0xFF &(~(tx_pwr_lvl - 1));
    }
    else {
        tx_pwr_lvl = console_cmd_strtoul(argv[2], NULL, 10);
        if(tx_pwr_lvl > 0x7F) {
...
int do_connect (int argc, char *argv[])
{
    ...
    if ((argc == 2) || (flash_wr_flag & ~0x01UL)) {
...
int fhost_console_iperf(char *params)
{
    ...
    fhost_iperf_start(&iperf_settings, &iperf_task_handle);
    return FHOST_NW_SUCCESS;
}
#endif /* PLF_IPERF */
