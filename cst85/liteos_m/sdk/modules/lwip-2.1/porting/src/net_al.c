...
static err_t net_if_init(struct netif *net_if)
{
    ...
    net_if->name[ 0 ] = 'w';
    net_if->name[ 1 ] = 'l';
    net_if->full_name[0] = 'w';
    net_if->full_name[1] = 'l';
    net_if->full_name[2] = '0' + net_num++;
    net_if->full_name[3] = '\0';
...
err_t net_eth_receive(struct pbuf *pbuf, struct netif *netif)
{
    ...
    int ret = sys_mbox_trypost(&conn->recvmbox, buf);
    if (ret != ERR_OK)
    {
        netbuf_delete(buf);
        return ERR_OK;
...
int lwip_sockopt_hook(int s, struct lwip_sock *sock, int level, int optname, void *optval, uint32_t *optlen, int *err)
{
    if (level == SOL_SOCKET && optname == SO_CONNINFO) {
        if ((sock->conn == NULL) || (*optlen < sizeof(void *))) {
            return EINVAL;
        }
