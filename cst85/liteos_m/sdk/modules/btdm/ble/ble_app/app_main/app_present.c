...
#include "app_present_task.h"                // Application task Definition
#include "app_present.h"                     // Application Definition
#include "hal_gap.h"                     // GAP Definition
#include "hal_gapm.h"                    // GAP Manager Definition
#include "hal_gapm_task.h"               // GAP Manager Task API
#include "hal_gapc_task.h"               // GAP Controller Task API

#include "bt_common.h"                   // Common BT Definition
#include "wb_co_math.h"                 // Common Maths Definition
 ...
 * x00\x60\x41\x49\x43\x42\x4C\x45 - "CSBLE"
 * --------------------------------------------------------------------------------------
 */
...
#if APP_BLE_USE_OPLIST
    app_ble_oplist_insert(GAPM_DELETE_ALL_ACTIVITIES,0,0,p_cmd);
#else
    // Send the message
    ke_msg_send(p_cmd);
...
void ble_enable(void)
{
    ...
    ASSERT_ERR(ke_state_get(TASK_APP) == APPM_INIT);
...
{
    if ((app_env.adv_state != APP_ADV_STATE_STARTED)
        && (app_env.conhdl == 0) && (app_env.conidx == 0)) {
       return false;
    } else {
        return true;
    }
...
{
    if (app_env.conidx || app_env.conhdl) {
        appm_disconnect();
        return CO_ERROR_NO_ERROR;
    } else {
...
uint8_t app_ble_update_con_params(uint16_t conn_intv_min, uint16_t conn_intv_max,
...
{
    bt_initializeList(&ble_app_oplist);
}

void app_ble_check_oplist(void)
{
    if(!bt_listIsEmpty(&ble_app_oplist) && app_env.adv_op == GAPM_NO_OP){
        BleAppOp *op = (BleAppOp *)bt_removeHeadList(&ble_app_oplist);
        // Send the message
...
void app_ble_oplist_insert(uint8_t optype ,uint8_t adv_state ,uint8_t adv_state_prv ,void *cmdptr)
{
    ...
    bt_insertTailList(&ble_app_oplist,&op->node);
    app_ble_check_oplist();
}
