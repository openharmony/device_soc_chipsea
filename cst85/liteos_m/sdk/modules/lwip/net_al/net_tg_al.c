        ...
        const ip_addr_t *addr, u16_t port)
{
    if (p != NULL)
    {
        ...
        pbuf_free(p);
        cs_time_get(SINCE_EPOCH, &actual.sec, &actual.usec);
        ...
        const ip_addr_t *addr, u16_t port)
{
    if (p != NULL)
    {
        ...
        cs_time_get(SINCE_EPOCH, &recv.sec, &recv.usec);
        ...
        struct fhost_ping_stream *my_stream)
{
    ...
    cs_time_get(SINCE_EPOCH, &send.sec, &send.usec);
...
int net_tg_send_transc(struct fhost_tg_stream *my_stream)
{
    ...
    cs_time_get(SINCE_EPOCH, &init.sec, &init.usec);
...
int net_tg_send(struct fhost_tg_stream *my_stream)
{
    ...
    while (my_stream->active && duration_cur < duration)
    {
        ...
        {
            cs_time_get(SINCE_EPOCH, &before.sec, &before.usec);
        ...
        if (throttled_rate != 0)
        {
            ...
            {
                cs_time_get(SINCE_EPOCH, &after.sec, &after.usec);
