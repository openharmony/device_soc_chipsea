...
void net_iperf_tcp_close(struct fhost_iperf_stream *stream)
{
    ...
    cs_time_get(SINCE_EPOCH, &now.sec, &now.usec);
                                   ...
                                   struct pbuf *p, err_t err)
{
    ...
    cs_time_get(SINCE_EPOCH, &report->packet_time.sec, &report->packet_time.usec);
                                ...
                                struct fhost_iperf_settings* iperf_settings)
{
    ...
    {
        write_more = 1;
        cs_time_get(SINCE_EPOCH, &packet_time.sec, &packet_time.usec);
        ...
        err_t err)
{
    ...
    cs_time_get(SINCE_EPOCH, &report->start_time.sec, &report->start_time.usec);
            ...
            if (iperf_timerbefore(&elapsed_time, &expected_sleep_time))
            {
                ...
                if (iperf_settings->interval.sec > 0 || iperf_settings->interval.usec > 0)
                {
                    ...
                    if (iperf_timerbefore(&elapsed_time, &expected_sleep_time))
                    {
                        ...
                        cs_time_get(SINCE_EPOCH, &last_sleep_time.sec, &last_sleep_time.usec);
                    }
                }
                else
                {
