...
#ifdef CFG_BLE_STACK
#define CS_BT_STACK_SIZE 1024*4
#else
#define CS_BT_STACK_SIZE TASK_STACK_SIZE_BT_TASK
#endif
...
/*
    ...
    // Notify the bt task
    rtos_task_notify(bt_task_handle, value, isr);
}
...
int bt_task_init(void)
{
    ...
    if (rtos_task_create(bt_task, "BT_TASK", BT_TASK,
                         CS_BT_STACK_SIZE, NULL, TASK_PRIORITY_MAX, &bt_task_handle)) {
...
#endif
    sysctrl_clock_cfg(CLK_CFG_D52S52P26F26);
