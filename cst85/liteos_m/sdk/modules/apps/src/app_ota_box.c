...
#define CS_OTA_ADDR_LIST_NUM   10
#define CS_OTA_RESEND_TIME     10

static BT_ADDR cs_ota_addr_list[CS_OTA_ADDR_LIST_NUM];
static uint8_t cs_ota_addr_num = 0;
const BT_ADDR cs_bt_addr = {{00,00,00,0x88,0x88,0x33}};
...
bool app_ota_check_cs_addr(BT_ADDR *ota_addr)
{
    if(ota_addr->addr[5] == cs_bt_addr.addr[5]
    && ota_addr->addr[4] == cs_bt_addr.addr[4]
    && ota_addr->addr[3] == cs_bt_addr.addr[3])
       return TRUE;
    else
       return FALSE;
}
...
{
    for(uint8_t i=0;i<CS_OTA_ADDR_LIST_NUM;i++){
        if(memcmp(ota_addr->addr,cs_bt_addr.addr,6) == 0){
            return;
        }
    }
    memcpy(cs_ota_addr_list[cs_ota_addr_num++].addr,ota_addr->addr,6);
}

void app_ota_clean_addr_list(void)
{
    memset((uint8_t *)cs_ota_addr_list,0,(CS_OTA_ADDR_LIST_NUM*6));
}
...
void app_ota_show_addr_list(void)
{
    if(cs_ota_addr_num){
        ...
        for(uint8_t i=0;i<cs_ota_addr_num;i++){
            BT_ADDR *oaddr = &cs_ota_addr_list[i];
...
void app_ota_connect_device(BT_ADDR * bdaddr)
{
    ...
    cs_adp_test_connect(bdaddr);
}

void app_ota_connect_by_list_id(uint8_t id)
{
    app_ota_connect_device(&cs_ota_addr_list[id]);
}

void app_ota_disconnect_device(void)
{
    cs_adp_test_disconnect();
}
...
void app_ota_resend_update_bin(uint32_t num)
{
    ...
    if(ret == BTDEF_PENDING){
        ...
        if(app_ota_resend_timer == NULL){
            co_timer_start(&app_ota_resend_timer,CS_OTA_RESEND_TIME,(void *)num,app_ota_bin_send_timer,0);
        }
    }else{
        //error occured
    }
...
{
    return cs_adp_test_send_data(data,len,header);
}
        ...
        case OTA_BY_VENDOR:
            {
                if(rxbuf[0] == OTA_INFO_HEADER){
                    ...
                    ota_start((OTA_ADDR + OTA_INFO_SIZE), total_len);
...
{
    CS_ADP_TEST_EVENT *cs_ota_msg = (CS_ADP_TEST_EVENT *)Event->Param;

    switch(Event->EventId){
        case CS_ADP_TEST_CONNECTED_CNF:
            TRACE("APP:ota_connect_cnf reason = %d\n",cs_ota_msg->testparam);
            app_ota_set_access(OTA_BY_VENDOR);
            break;
        case CS_ADP_TEST_DISCONNECTED:
            TRACE("APP:ota_disconnect reason = %d\n",cs_ota_msg->testparam);
            app_ota_set_access(OTA_BY_VENDOR);
            break;
#if APP_SUPPORT_OTA_BOX
        case CS_ADP_TEST_INQUIRY_RESULT:
            if(app_ota_check_cs_addr(&cs_ota_msg->bdaddr)){
                app_ota_add_addr_list(&cs_ota_msg->bdaddr);
            }
            break;
        case CS_ADP_TEST_INQUIRY_COMPLETE:
            app_ota_show_addr_list();
            break;
        case CS_ADP_TEST_INQUIRY_CANCELED:
            app_ota_show_addr_list();
            break;
#endif
        case CS_ADP_TEST_L2CAP_CONNECTD:
...
#endif
            break;
        case CS_ADP_TEST_L2CAP_DISCONNECTD:
...
#endif
            break;
        case CS_ADP_TEST_L2CAP_DATA_SENT:
            {
                TestTxDone *tx_done = (TestTxDone *)cs_ota_msg->testparam;
            ...
            break;
        case CS_ADP_TEST_L2CAP_DATA_IND:
            {
                TestRxDone *rx_done = (TestRxDone *)cs_ota_msg->testparam;
