...
void app_hfp_cclk_check_timer( TimerHandle_t xTimer )
{
    if (!at_cclk_acked) {
        ...
        struct tm tim_st;
        char buffer[48] = {0,};
        cs_time_get(SINCE_EPOCH, &tv_sec, &tv_usec);
        gmtime_offset_r((const time_t *)&tv_sec, &tim_st, 8);
        dbg_snprintf(buffer, sizeof(buffer), "BJ TIME: %d-%02d-%02d %02d:%02d:%02d\r\n",
            tim_st.tm_year + YEAR0,tim_st.tm_mon,tim_st.tm_mday,tim_st.tm_hour,tim_st.tm_min,tim_st.tm_sec);
        uart_puts(buffer);
        #if CFG_CUR_TIME_INFORMER
        uart1_puts(buffer);
        #endif
...
{
    cs_adp_hfp_send_at_cclk(bdaddr);
...
{
    cs_adp_hfp_send_siri_report(bdaddr);
    return 0;
}

int app_hfp_siri_voiceRecognition(BT_ADDR bdaddr)
{
    cs_adp_hfp_send_voice_recognition(bdaddr, 1);
    return 0;
}
...
void app_hfp_play(BT_ADDR bdaddr)
{
    ...
    scohandle = cs_adp_hfp_get_scohandle_by_addr(bdaddr);
    if(scohandle){
        app_set_hfp_state(APP_PLAY_SCO);
        negotiated_codec = cs_adp_hfp_get_negotiated_codec_by_addr(bdaddr);
...
void app_handle_hfp_connect_status(CS_EVENT *event)
{
    CS_ADP_HFP_EVENT *cs_hfp_msg = (CS_ADP_HFP_EVENT *)event->Param;
    app_connect_manager * app_bt_con = NULL;

    app_bt_con = app_bt_get_con_ptr_by_addr(&cs_hfp_msg->bdaddr);
    if(!app_bt_con ){
        ...
        memcpy(app_bt_con->remotedev_addr.addr,cs_hfp_msg->bdaddr.addr, BD_ADDR_SIZE);
    }
    ...
    switch(event->EventId){
        case CS_ADP_HFP_CONNECTED:
            {
                btdev_info* dev_info = cs_adp_get_devinfo_by_addr(&cs_hfp_msg->bdaddr);

                if(cs_hfp_msg->p.conP.reason){
                    return;
                }
                if(dev_info){
                    dev_info->hf_state = 1;
        ...
        case CS_ADP_HFP_DISCONNECTED:
            {
                ...
                switch(app_bt_con->reconnect.reconnect_mode){
                    ...
                    default:
                        ...
                        if(cs_hfp_msg->p.discP.reason == BT_CONNECTION_TIMEOUT){
...
void app_hfp_state_machine(CS_EVENT *event)
{
    CS_ADP_HFP_EVENT *cs_hfp_msg = (CS_ADP_HFP_EVENT *)event->Param;
    switch(event->EventId)
    {
        ...
        case CS_ADP_HFP_DISCONNECTED:
            ...
            break;
        case CS_ADP_HFP_AUDIO_CONNECTED:
            {
                uint16_t scohandle = 0;
                uint16_t negotiated_codec = 0;

                scohandle = cs_adp_hfp_get_scohandle_by_addr(cs_hfp_msg->bdaddr);
                if(scohandle){
                    negotiated_codec = cs_adp_hfp_get_negotiated_codec_by_addr(cs_hfp_msg->bdaddr);
                    switch (negotiated_codec)
                    {
                        case MEDIA_SCO_CODEC_CVSD:
            ...
            break;
        case CS_ADP_HFP_AUDIO_DISCONNECTED:
            {
                app_set_hfp_scocodec(MEDIA_SCO_CODEC_NONE);
            }
            break;
        case CS_ADP_HFP_CALL_STATUS_IND:
            {
                bt_hfp_adp_call_status call_state = cs_hfp_msg->p.callP.callS;
            ...
            break;
        case CS_ADP_HFP_CALLSETUP_STATUS_IND:
            {
                bt_hfp_adp_call_status call_state = cs_hfp_msg->p.callsetupP.callS;
                switch(call_state){
            ...
            break;
        case CS_ADP_HFP_CALLHOLD_STATUS_IND:
            {
                bt_hfp_adp_callhold_status hfp_call_hold = cs_hfp_msg->p.callholdP.callholdS;
...
BOOL app_hfp_key_handle(uint32_t key)
{
    ...
    hfp_devinfo = cs_adp_get_hfp_current_devinfo();
    if(hfp_devinfo == NULL){
        return ret;
    }
    ...
    switch(key){
        case APP_KEY_PLAY|APP_KEY_PRESS:
            {
                ...
                if(hfp_devinfo->hfp_state == HFP_ADP_STATE_CONNECTED){
                    if(hfp_devinfo->hfp_call_status == BT_CALL_STATUS_ACTIVE ||
                       ...
                       cs_adp_hfp_get_voicerecognition_state(hfp_devinfo->bdaddr)){
                            cs_adp_hfp_call_release(hfp_devinfo->bdaddr);
                            ret = TRUE;
                    }else if(hfp_devinfo->hfp_call_status == BT_CALL_STATUS_INCOMING){
                            cs_adp_hfp_call_answer(hfp_devinfo->bdaddr);
                            ret = TRUE;
                    }
                }
            }
        ...
        case APP_KEY_VOLSUB|APP_KEY_HOLD:
            ...
            if(hfp_devinfo->hfp_call_status != BT_CALL_STATUS_NONE){
                cs_adp_hfp_send_volume(hfp_devinfo->bdaddr ,app_get_hfp_volume(&hfp_devinfo->bdaddr));
                ret = TRUE;
            }
            break;
        case APP_KEY_PLAY|APP_KEY_DOUBLE_CLICK:
            ...
            {
                cs_adp_hfp_call_redial(hfp_devinfo->bdaddr);
                ret = TRUE;
            }
            break;
...
void app_hfp_msg_handle(CS_EVENT *event)
{
    ...
    switch(event->EventId)
    {
        case CS_ADP_HFP_CONNECTED:
            ...
            if(cs_hfp_msg->p.conP.reason == BT_NO_ERROR){
                if(cs_hfp_msg->role == HANDSFREE){
#if APP_SUPPORT_NREC_OFF == 1
                    cs_adp_hfp_disable_NREC(cs_hfp_msg->bdaddr);
#endif
                    cs_adp_hfp_set_battery_level(7);
                    cs_adp_hfp_send_volume(cs_hfp_msg->bdaddr ,app_get_hfp_volume(&cs_hfp_msg->bdaddr));
                    app_hfp_siri_report(cs_hfp_msg->bdaddr);
                    app_hfp_send_at_cclk(cs_hfp_msg->bdaddr);
                }else{
            ...
            app_bt_handler_register(HANDLER_REG_3,cs_adp_hfp_battery_report_proc);
            app_handle_hfp_connect_status(event);
            break;
        case CS_ADP_HFP_DISCONNECTED:
            {
                uint8_t discReason = (uint8_t)cs_hfp_msg->p.discP.reason;
        ...
        case CS_ADP_HFP_AUDIO_CONNECTED:
            ...
            if(cs_hfp_msg->p.scoP.reason == BT_NO_ERROR)
                app_hfp_play(cs_hfp_msg->bdaddr);
#endif
            break;
        case CS_ADP_HFP_AUDIO_DISCONNECTED:
#if PLF_USB_BT == 0
...
#endif
            break;
        case CS_ADP_HFP_AUDIO_DATA_SENT:

            break;
        case CS_ADP_HFP_AUDIO_DATA_IND:
            {
                CS_ADP_HFP_AUDIO_DATA *audiodata = (CS_ADP_HFP_AUDIO_DATA *)cs_hfp_msg->p.dataindP.buff;
                CS_ADP_HFP_AUDIO_DATA sendaudiodata;
                ...
                if(sendaudiodata.len)
                    cs_adp_hfp_send_sco_data(cs_hfp_msg->bdaddr,&sendaudiodata);
                hf_sendbuff.idx++;
#else
        ...
        case CS_ADP_HFP_CALLHOLD_STATUS_IND:
            ...
            break;
        case CS_ADP_HFP_RING_IND:

            break;
        case CS_ADP_HFP_SIRI_STATUS:
            TRACE("APP: siri status = %d\n",cs_hfp_msg->p.siriP.siriS);
            break;
        case CS_ADP_HFP_CURRENT_CALL_NUM:

            break;
        case CS_ADP_HFP_SPEAKER_VOLUME:
            {
                U8 volume = (U8)cs_hfp_msg->p.volP.volume;
                TRACE("APP:HFP_SPEAKER_VOLUME = %d\n",volume);
                app_set_hfp_volume(&cs_hfp_msg->bdaddr,volume);
#if APP_SUPPORT_TWS == 0
                app_bt_local_volume_handle(1,app_get_hfp_volume(&cs_hfp_msg->bdaddr));
#endif
            }
            break;
        case CS_ADP_HFP_COMMON_AT_RESULT_IND:
            {
                U8 *at_resualt_string = (U8 *)cs_hfp_msg->p.atP.at_result;
