...
int app_bt_queue_process(APP_BT_MSG* msg)
{
    ...
    switch (msg->msg_id) {
        case BT_ROLE_SWITCH:
            status = cs_adp_role_switch(msg->param.p.bdaddr);
            break;
        case BT_DISCONNECT_LINK:
            cs_adp_disconnect_acl(msg->param.p.bdaddr);
        ...
        case BT_DISCONNECT_LINK_ALL:
            ...
            break;
        case BT_STOP_SNIFF:
            status = cs_adp_stop_sniff(msg->param.p.bdaddr);
            break;
        case BT_SET_SCAN_MODE:
            status = cs_adp_set_scan_mode(msg->param.p.mode);
            break;
        case BT_SET_LINK_POLICY:
            status = cs_adp_set_linkpolicy(msg->param.p.linkpolicy_param.bdaddr,
                                      msg->param.p.linkpolicy_param.policy);
            break;
        case BT_SET_SNIFF_TIMER:
            if (msg->param.p.btsniff_param.sniff_info.maxInterval == 0){
                status = cs_adp_set_sniff_timer(msg->param.p.btsniff_param.bdaddr,
                                            NULL,
        ...
        case BT_A2DP_CONNECT:
            status = cs_adp_a2dp_connect(msg->param.p.bdaddr);
            break;
        case BT_A2DP_DISCONNECT:
            status = cs_adp_a2dp_disconnect(msg->param.p.bdaddr);
            break;
#endif
#if APP_SUPPORT_HFP == 1
        case BT_HFP_CONNECT:
            status = cs_adp_hf_connect(msg->param.p.bdaddr);
            break;
        case BT_HFP_DISCONNECT:
            status = cs_adp_hf_disconnect(msg->param.p.bdaddr);
            break;
        case BT_HFP_SCO_CONNECT:
            status = cs_adp_hf_sco_connect(msg->param.p.bdaddr);
            break;
        case BT_HFP_SCO_DISCONNECT:
            status = cs_adp_hf_sco_disconnect(msg->param.p.bdaddr);
            break;
#endif
#if APP_SUPPORT_HSP == 1
        case BT_HSP_CONNECT:
            status = cs_adp_hs_connect(msg->param.p.bdaddr);
            break;
        case BT_HSP_SCO_CONNECT:
            status = cs_adp_hs_sco_connect(msg->param.p.bdaddr);
            break;
        case BT_HSP_SCO_DISCONNECT:
            status = cs_adp_hs_sco_disconnect(msg->param.p.bdaddr);
            break;
        case BT_HSP_DISCONNECT:
            status = cs_adp_hs_disconnect(msg->param.p.bdaddr);
            break;
#endif
#if APP_SUPPORT_A2DP_SOURCE == 1
        case BT_A2DP_SOURCE_CONNECT:
            status = cs_adp_a2dp_source_connect(msg->param.p.bdaddr);
            break;
...
#if APP_SUPPORT_HFG
        case BT_HFG_CONNECT:
            cs_adp_hfg_connect(msg->param.p.bdaddr);
            break;
#endif
#if APP_SUPPORT_TWS == 1
        case BT_TWS_CONNECT:
        ...
        case BT_TWS_SYNC_CTRL:
            cs_adp_host_control_tws_sync_con((BOOL)msg->param.p.tws_param.tws_sync_ctrl);
            break;
        case BT_TWS_MSS_CTRL:
            app_tws_begin_mss();
            break;
        case BT_TWS_STOP_LOCAL_SYNC_CTRL:
            cs_adp_host_sync_stop();
            break;
        case BT_TWS_SNED_SYNC_PARAM:
            cs_adp_set_audio_sync_param(msg->param.p.sync_param.current_clk,msg->param.p.sync_param.counterNum ,\
            ...
            break;
        case BT_TWS_RESET:
            cs_adp_update_tws_flash_role(TWS_NOROLE);
            app_bt_disconnect_all();
            break;
        case BT_TWS_DISC:
        ...
        case BT_BOTH_SCAN:
            ...
            break;
        case BT_INQUIRY_DEV:
            cs_adp_inquiry(msg->param.p.inquiry_param.len,msg->param.p.inquiry_param.maxResp);
            break;
        case BT_INQUIRY_CANCEL:
            cs_adp_inquiry_cancel();
            break;
#if APP_SUPPORT_OTA_BOX
        case BT_OTA_CONNECT:
