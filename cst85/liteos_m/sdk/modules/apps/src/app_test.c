...
void app_test_send_1K_sco_data(U16 len)
{
    ...
    cs_adp_test_send_sco_data(&sendaudiodata);
    send_sco_counter++;
}

void app_test_msg_handle(CS_EVENT *event)
{
    CS_ADP_TEST_EVENT *cs_test_msg = (CS_ADP_TEST_EVENT *)event->Param;
    switch(event->EventId){
        case CS_ADP_TEST_CONNECTED_CNF:
            TRACE("APP:test_connect_cnf status = %d\n",cs_test_msg->testparam);
        ...
        case CS_ADP_TEST_SCO_CONNECT:
            ...
            break;
        case CS_ADP_TEST_SCO_DISCONNECT:
            TRACE("APP:test_sco_disconnect status = %d\n",cs_test_msg->testparam);
            break;
        case CS_ADP_TEST_SCO_DATA_SENT:
            TRACE("APP:test sco send ok\n");
            break;
        case CS_ADP_TEST_SCO_DATA_IND:
            {
                CS_ADP_HFP_AUDIO_DATA *audiodata = (CS_ADP_HFP_AUDIO_DATA *)cs_test_msg->testparam;
                CS_ADP_HFP_AUDIO_DATA sendaudiodata;
