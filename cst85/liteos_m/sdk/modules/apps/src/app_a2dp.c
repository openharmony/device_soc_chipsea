...
void app_a2dp_play(BT_ADDR bdaddr)
{
    ...
    audio_cap = cs_adp_a2dp_get_audio_cap_by_addr(bdaddr);
...
void app_handle_a2dp_connect_status(CS_EVENT *event)
{
    CS_ADP_A2DP_EVENT *cs_a2dp_msg = (CS_ADP_A2DP_EVENT *)event->Param;
    app_connect_manager * app_bt_con = NULL;

    app_bt_con = app_bt_get_con_ptr_by_addr(&cs_a2dp_msg->bdaddr);
    if(!app_bt_con ){
        ...
        memcpy(app_bt_con->remotedev_addr.addr,cs_a2dp_msg->bdaddr.addr, BD_ADDR_SIZE);
    }
    ...
    switch(event->EventId){
        case CS_ADP_A2DP_STREAM_OPEN:
            {
                btdev_info* dev_info = cs_adp_get_devinfo_by_addr(&cs_a2dp_msg->bdaddr);
                if(dev_info){
                    ...
                    dev_info->a2dp_role = cs_a2dp_msg->role;
                }
            ...
            break;
        case CS_ADP_A2DP_STREAM_CLOSED:
            {
                ...
                switch(app_bt_con->reconnect.reconnect_mode){
                    ...
                    case RECONNECT_AFTER_DISC:
                        ...
                        break;
                    default:
                        if(cs_a2dp_msg->p.disc_reason == BT_CONNECTION_TIMEOUT){
...
void app_a2dp_state_machine(CS_EVENT *event)
{
    //CS_ADP_A2DP_EVENT *cs_a2dp_msg = (CS_ADP_A2DP_EVENT *)event->Param;
...
void app_a2dp_msg_handle(CS_EVENT *event)
{
    ...
    switch(event->EventId) {
        case CS_ADP_A2DP_STREAM_OPEN:
            {
                if(cs_a2dp_msg->role == BT_A2DP_SOURCE){
#if APP_SUPPORT_A2DP_SOURCE == 1
#endif
                }
            ...
            break;
        case CS_ADP_A2DP_STREAM_STARTED:
            {
                A2DP_AUDIO_CAP *audio_cap = (A2DP_AUDIO_CAP *)cs_a2dp_msg->p.signalingP.audioCodecP;
                ...
                if(cs_a2dp_msg->role == BT_A2DP_SOURCE){
                    ...
                    app_a2dp_play(cs_a2dp_msg->bdaddr);
                }
            }
            break;
        case CS_ADP_A2DP_STREAM_SUSPENDED:
            {
                TRACE("APP:a2dp_stream_suspended role = %d\n",cs_a2dp_msg->role);
                if(cs_a2dp_msg->role == BT_A2DP_SINK){
                    app_a2dp_pause();
                }
            }
            break;
        ...
        case CS_ADP_A2DP_STREAM_PACKET_SENT:
            ...
            break;
        case CS_ADP_A2DP_STREAM_DATA_IND:
            {
