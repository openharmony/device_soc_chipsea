...
#define AUDIO_TWS_OFFSET        128
#define VOICE_TWS_MSBC_OFFSET   128
#define VOICE_TWS_CVSD_OFFSET   128
#endif

#if TWS_MSS_AUTO_TEST == 1
TimerHandle_t app_bt_tws_mss_timer = NULL;
#endif
#if TWS_SENDING_DATA_TEST == 1
TimerHandle_t app_bt_tws_test_send_timer = NULL;
U8 send_test[6] = {1,2,3,4,5,6};
#endif
...
#else
    cs_adp_connect_tws_peer_device(&temp_bdaddr);
#endif
}

#if TWS_SENDING_DATA_TEST == 1
void app_tws_start_test_send(void)
{
    TRACE("APP:app_tws_start_test_send.\n");
    if(rtos_timer_change_period(app_bt_tws_test_send_timer, 100, 0) == 0){

    }
}

void app_tws_test_send_timer( TimerHandle_t xTimer )
{
    TRACE("APP:app_tws_test_send_timer.\n");
    app_bt_tws_send_data(send_test,6);
    app_tws_start_test_send();
}

void app_tws_create_test_send_timer(void)
{
    if(app_bt_tws_test_send_timer == NULL)
        app_bt_tws_test_send_timer = rtos_timer_create("APP_TWS_TEST_SEND",      \
                                                     10000,      \
                                                     pdFALSE,\
                                                     NULL,\
                                                     app_tws_test_send_timer);
}
#endif
...
{
    return cs_adp_send_data(ptrData, length);
}
...
void app_tws_set_trig_num(AppTwsMediaDataStruct *trig_num)
{
    uint32_t protect;
    MediaCommonStruct tmp;
    rwip_time_t bt_clk;
    ...
    bt_clk = cs_adp_get_native_clk_plus_offset();
    ...
    TRACE("APP:triger hs = 0x%x hus = %d\n",bt_clk.hs ,bt_clk.hus);
...
void app_tws_msg_handle(CS_EVENT *event)
{
    ...
    switch(event->EventId){
        case CS_ADP_TWS_CONNECTED_IND:
            {
                CS_ADP_TWS_EVENT *cs_adp_tws_msg = (CS_ADP_TWS_EVENT *)event->Param;
                TRACE("APP:app_tws_connect_ind .status = %d\n",cs_adp_tws_msg->p.conP.reason);
            }
            break;
        case CS_ADP_TWS_CONNECTED_CNF:
            {
                CS_ADP_TWS_EVENT *cs_adp_tws_msg = (CS_ADP_TWS_EVENT *)event->Param;
                ...
                app_tws_start_test_send();
#endif
#if PXP == 1 && TWS_SENDING_DATA_TEST == 0
                app_bt_set_scan_mode(BT_CONNECTABLE);
#endif
                if(cs_adp_tws_msg->p.conP.reason){
#if TWS_BLE_CONNECT == 1
                    cs_adp_tws_set_scan_para(0);
                    app_tws_set_ble_state(STATE_SCAN_EN);
#endif
        ...
        case CS_ADP_TWS_DISCONNECTED:
            {
                ...
                if(tws_app_get_flash_role() < TWS_NOROLE){
                    switch(cs_adp_tws_msg->p.discP.last_role){
                        case TWS_SLAVE:
#if TWS_BLE_CONNECT == 1
                            app_tws_pause_a2dp();
                            app_tws_stop_hfp();
            ...
            break;
        case CS_ADP_TWS_STREAM_STARTED:
            {
                CS_ADP_A2DP_EVENT *cs_a2dp_msg = (CS_ADP_A2DP_EVENT *)event->Param;
                A2DP_AUDIO_CAP *audio_cap = (A2DP_AUDIO_CAP *)cs_a2dp_msg->p.signalingP.audioCodecP;
                app_tws_play_a2dp(audio_cap);
            }
            break;
        case CS_ADP_TWS_STREAM_SUSPENDED:
            {
                app_tws_pause_a2dp();
            }
            break;
        case CS_ADP_TWS_STREAM_DATA_IND:
            {
                CS_ADP_A2DP_EVENT *cs_a2dp_msg = (CS_ADP_A2DP_EVENT *)event->Param;
                A2DP_AUDIO_CAP *audio_cap = (A2DP_AUDIO_CAP *)cs_a2dp_msg->p.mediaP.audioCodecP;
                A2DP_AUDIO_DATA *audio_data = (A2DP_AUDIO_DATA *)cs_a2dp_msg->p.mediaP.audioDataP;
        ...
        case CS_ADP_TWS_SCO_CONNECT:
            {
                ...
                app_tws_play_hfp((U16)cs_hfp_msg->p.scoP.negotiated_codec);
            }
            break;
        case CS_ADP_TWS_SCO_DISCONNECT:
            {
                app_tws_stop_hfp();
            }
            break;
        case CS_ADP_TWS_SCO_DATA_IND:
            {
                CS_ADP_HFP_EVENT *cs_hfp_msg = (CS_ADP_HFP_EVENT *)event->Param;
            ...
            break;
        case CS_ADP_TWS_TRIG_NUM:
            {
                CS_ADP_TWS_EVENT *cs_adp_tws_msg = (CS_ADP_TWS_EVENT *)event->Param;
                app_tws_set_trig_num((AppTwsMediaDataStruct *)cs_adp_tws_msg->p.trigP.tws_trig_data);
            }
            break;
        case CS_ADP_TWS_AUDIO_SYNC_MSG:
            {
                CS_ADP_TWS_EVENT *cs_adp_tws_msg = (CS_ADP_TWS_EVENT *)event->Param;
                app_tws_set_audio_sync_param((AppTwsMediaDataStruct *)cs_adp_tws_msg->p.syncP.tws_sync_data);
            }
            break;
        case CS_ADP_TWS_ADV_DATA:
            {
#if TWS_BLE_CONNECT == 1
                ...
                cs_adp_connect_tws_peer_device(&remove_addr);
#endif
            }
            break;
        case CS_ADP_TWS_LOST_SEQUENCE:
            {
                app_bt_send_bt_audio_restart();
            }
            break;
        case CS_ADP_TWS_DATA_SEND:
            {
                CS_ADP_TWS_EVENT *cs_adp_tws_msg = (CS_ADP_TWS_EVENT *)event->Param;
                host_data_struct *tx_done = (host_data_struct *)cs_adp_tws_msg->p.datasendP.buff;
            ...
            break;
        case CS_ADP_TWS_DATA_IND:
            {
                CS_ADP_TWS_EVENT *cs_adp_tws_msg = (CS_ADP_TWS_EVENT *)event->Param;
                host_data_struct *rx_done = (host_data_struct *)cs_adp_tws_msg->p.dataindP.buff;
#if TWS_SENDING_DATA_TEST == 1
...
#endif
#if TWS_SENDING_DATA_TEST == 1
    app_tws_create_test_send_timer();
#endif
}
#endif
